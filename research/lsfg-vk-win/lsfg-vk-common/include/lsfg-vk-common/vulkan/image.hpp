...
namespace vk {
    ...
    public:
        ...
        /// @param importHandle optional external handle for shared memory
        /// @param exportHandle optional pointer where an exported handle will be stored
        /// @throws ls::vulkan_error on failure
        Image(const vk::Vulkan& vk,
            VkExtent2D extent,
            VkFormat format = VK_FORMAT_R8G8B8A8_UNORM,
            VkImageUsageFlags usage = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            std::optional<ExternalHandle> importHandle = std::nullopt,
            std::optional<ExternalHandle*> exportHandle = std::nullopt);
