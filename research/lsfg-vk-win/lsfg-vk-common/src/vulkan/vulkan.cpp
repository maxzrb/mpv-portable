...
namespace {
    /// load the platform Vulkan loader and return its handle
    void* get_vulkan_handle() {
        ...
        if (!handle)
            throw ls::vulkan_error("failed to load vulkan-1.dll");
#else
    ...
    PFN_vkGetInstanceProcAddr get_mpa() {
        ...
        mpa = reinterpret_cast<PFN_vkGetInstanceProcAddr>(
            GetProcAddress(reinterpret_cast<HMODULE>(get_vulkan_handle()),
...
#if defined(_WIN32)
        .GetMemoryWin32HandleKHR = dpa<PFN_vkGetMemoryWin32HandleKHR>(f, d,
            "vkGetMemoryWin32HandleKHR"),
        .ImportSemaphoreWin32HandleKHR = dpa<PFN_vkImportSemaphoreWin32HandleKHR>(f, d,
            "vkImportSemaphoreWin32HandleKHR"),
        .GetSemaphoreWin32HandleKHR = dpa<PFN_vkGetSemaphoreWin32HandleKHR>(f, d,
