...
namespace lsfgvk::backend {
    ...
    public:
        ...
        ContextImpl(const InstanceImpl& instance,
            std::pair<vk::ExternalHandle, vk::ExternalHandle> sourceHandles,
            const std::vector<vk::ExternalHandle>& destHandles,
    ...
    std::filesystem::path findCacheFilePath() {
        ...
        if (localAppData && *localAppData != '\0')
            return std::filesystem::path(localAppData) /
                "lsfg-vk" / "lsfg-vk_pipeline_cache.bin";

        return std::filesystem::current_path() / "lsfg-vk_pipeline_cache.bin";
#else
...
namespace {
    ...
    std::pair<vk::Image, vk::Image> importImages(const vk::Vulkan& vk,
            const std::pair<vk::ExternalHandle, vk::ExternalHandle>& sourceHandles,
    ...
    std::vector<vk::Image> importImages(const vk::Vulkan& vk,
        ...
        try {
            std::vector<vk::Image> destImages;
            destImages.reserve(destHandles.size());

            for (const auto& externalHandle : destHandles)
    ...
    vk::TimelineSemaphore importTimelineSemaphore(const vk::Vulkan& vk,
            vk::ExternalHandle syncHandle) {
        try {
            return{vk, 0, syncHandle};
        } catch (const std::exception& e) {
...
ContextImpl::ContextImpl(const InstanceImpl& instance,
            ...
            vk::ExternalHandle syncHandle,
            VkExtent2D extent, bool hdr, float flow, bool perf) :
        sourceImages(importImages(instance.getVulkan(), sourceHandles,
            extent, hdr ? VK_FORMAT_R16G16B16A16_SFLOAT : VK_FORMAT_R8G8B8A8_UNORM)),
        destImages(importImages(instance.getVulkan(), destHandles,
            extent, hdr ? VK_FORMAT_R16G16B16A16_SFLOAT : VK_FORMAT_R8G8B8A8_UNORM)),
        blackImage(createBlackImage(instance.getVulkan())),
        syncSemaphore(importTimelineSemaphore(instance.getVulkan(), syncHandle)),
        prepassSemaphore(createPrepassSemaphore(instance.getVulkan())),
        cmdbufs(createCommandBuffers(instance.getVulkan(), destHandles.size() + 1)),
        cmdbufFence(instance.getVulkan()),
        ctx(createCtx(instance, extent, hdr, flow, perf, destHandles.size())),
