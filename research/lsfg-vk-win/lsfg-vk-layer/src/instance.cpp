...
namespace {
    void set_layer_disabled(bool disabled) {
#if defined(_WIN32)
        _putenv_s("DISABLE_LSFGVK", disabled ? "1" : "");
#else
        if (disabled)
...
void Root::createSwapchainContext(const vk::Vulkan& vk,
    ...
    std::cerr << "lsfg-vk: frame generation context ready ("
