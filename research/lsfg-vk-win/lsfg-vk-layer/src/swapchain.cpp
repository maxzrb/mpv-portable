...
Swapchain::Swapchain(const vk::Vulkan& vk, backend::Instance& backend,
    ...
    if (const auto* telemetry = std::getenv("LSFGVK_TELEMETRY_PATH");
            telemetry && *telemetry) {
        this->telemetryPath = std::filesystem::path(telemetry);
        std::cerr << "lsfg-vk: telemetry enabled at '"
                  << this->telemetryPath->string() << "'\n";
    }

    const VkExtent2D extent = this->info.extent;
    const bool hdr = this->info.format > 57;

    std::vector<vk::ExternalHandle> sourceHandles(2);
    std::vector<vk::ExternalHandle> destinationHandles(this->profile.multiplier - 1);

    this->sourceImages.reserve(sourceHandles.size());
    for (auto& externalHandle : sourceHandles)
        this->sourceImages.emplace_back(vk,
            extent, hdr ? VK_FORMAT_R16G16B16A16_SFLOAT : VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            std::nullopt, &externalHandle);

    this->destinationImages.reserve(destinationHandles.size());
    for (auto& externalHandle : destinationHandles)
        this->destinationImages.emplace_back(vk,
            extent, hdr ? VK_FORMAT_R16G16B16A16_SFLOAT : VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            std::nullopt, &externalHandle);

    vk::ExternalHandle syncHandle{};
    this->syncSemaphore.emplace(vk, 0, std::nullopt, &syncHandle);

    try {
        this->ctx = ls::owned_ptr<ls::R<backend::Context>>(
            new ls::R<backend::Context>(backend.openContext(
                { sourceHandles.at(0), sourceHandles.at(1) },
                destinationHandles, syncHandle,
...
void Swapchain::updateTelemetry() noexcept {
    ...
    const auto elapsed = std::chrono::duration<double>(
        now - this->telemetryWindowStart).count();
    if (elapsed < 0.5)
        return;

    auto inputFps = static_cast<double>(this->telemetryInputFrames) / elapsed;

    // Optimus iGPU / 无 VRR 显示器下 Present 速率可能等于屏幕刷新率而非源帧率。
    // 上层 Lua 脚本写入 estimated-vf-fps，由启动脚本设为环境变量。
    ...
    try {
        std::ofstream telemetry(
            *this->telemetryPath, std::ios::out | std::ios::trunc);
        telemetry.imbue(std::locale::classic());
        if (telemetry) {
            telemetry << std::fixed << std::setprecision(2)
                      ...
                      << ",\"performance\":"
                      << (this->profile.performance_mode ? "true" : "false")
    ...
    this->telemetryInputFrames = 0;
    this->telemetryOutputFrames = 0;
